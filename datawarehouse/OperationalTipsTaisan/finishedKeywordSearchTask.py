...
def upload_to_gs():
	...
	f = "finishedKeywordSearchTask.csv"
        SQL = """select * from finishedKeywordSearchTask""" 
	...
	cmd_csv = """mysql -heqx-taisanPost-subordinate-db -ukettle -pk3UTLe taisan_statistics -e"%s">> %s""" %(SQL,f)
	print cmd_csv
	cmd_upload = "gsutil cp %s gs://vobile-data-analysis/OperationalTipsTaisan/trackingMeta" %f
	load_bigquery = "bq load --skip_leading_rows=1 --field_delimiter='\t' --quote='' OperationalTipsTaisan.finishedKeywordSearchTask gs://vobile-data-analysis/OperationalTipsTaisan/trackingMeta/%s" %f 
	os.system(cmd_csv)
