...
def upload_to_gs():
    beg = datetime.date(2016,02,9)
    end = datetime.date(2016,06,02)
    for i in xrange((end- beg).days+1):
	date = str(end - datetime.timedelta(days=i))
	date1 = str(end - datetime.timedelta(days=i-1)) 
	f = "video%s.csv" %date.replace("-","")
	#f = "trackingWebsite_keyword.csv"
        SQL = """select * from video where created_at >= '%s' and created_at < '%s'""" %(date,date1) 
	...
	load_bigquery = "bq load --skip_leading_rows=1 --field_delimiter='\t' --quote='' OperationalTipsTaisan.video gs://vobile-data-analysis/OperationalTipsTaisan/trackingMeta/%s" %f 
	os.system(cmd_csv)
