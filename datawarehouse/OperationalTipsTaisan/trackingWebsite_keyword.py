...
def upload_to_gs():
    beg = datetime.date(2015,03,02)
    end = datetime.date(2016,02,23)
    for i in xrange((end- beg).days+1):
	date = str(end - datetime.timedelta(days=i))
	date1 = str(end - datetime.timedelta(days=i-1)) 
	f = "trackingWebsite_keyword%s.csv" %date.replace("-","")
	#f = "trackingWebsite_keyword.csv"
        SQL = """select * from trackingWebsite_keyword where created_at >= '%s' and created_at < '%s'""" %(date,date1) 
	...
	cmd_csv = """mysql -heqx-taisan-slave-db -ukettle -pk3UTLe taisan -e"%s">> %s""" %(SQL,f)
	print cmd_csv
	cmd_upload = "gsutil cp %s gs://vobile-data-analysis/OperationalTipsTaisan/trackingWebsite_keyword" %f
	load_bigquery = "bq load --skip_leading_rows=1 --field_delimiter='\t' --quote='' OperationalTipsTaisan.trackingWebsite_keyword gs://vobile-data-analysis/OperationalTipsTaisan/trackingWebsite_keyword/%s" %f 
	os.system(cmd_csv)
