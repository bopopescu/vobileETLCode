...
for i in range(0,11049):
  ...
  stop = stop + 1
colname = colname[0:-1]
get_data = "select * from video_trackingMetaFinished where video_id in (%s)" %(colname)
...
for e in rows:
   if not e is None:
      insert = 'insert into video_trackingMetaFinished(video_id,trackingMeta_id) values("%s","%s")' %(e[0],e[1])
