...
start = 0
stop = 1
conn=MySQLdb.connect(host='p2p-3-replica-02.c85gtgxi0qgc.us-west-1.rds.amazonaws.com',user='kettle',passwd='k3UTLe',port=3306)

for i in range(1,40):
# Disney
  cur=conn.cursor()
  conn.select_db('p2pwarehouseDisney')
  get_data = "select count(*) from infringmentSummary where created_at >= date_sub(curdate(),interval '%s' day) and created_at < date_sub(curdate(),interval '%s' day)" %(stop,start)
...
#Discovery_One
  ...
  update = "update TempMetricsReportAll set IPs_P2P = '%s' where reportedDate = date_sub(curdate(),interval '%s' day)" %(IPs,stop)
  print update
