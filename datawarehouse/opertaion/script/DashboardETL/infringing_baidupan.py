...
cur=conn.cursor()
get_data = "select b.id, date(a.created_at), count(*), sum(if(hide_flag=2,1,0)) infringing from matchedVideo a, mddb.trackingWebsite b where  a.created_at >= date_sub(curdate(),interval 1 day) and a.created_at < date_sub(curdate(),interval 0 day) and a.trackingWebsite_id = b.id and b.id = 1425 group by 1;"
