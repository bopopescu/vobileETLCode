...
for e in rows:
    update = "update TempMetricsReportAll set Compliance_Percent_Cyberlocker = '%s', Compliance_Time_Cyberlocker = '%s' where reportedDate = date_sub(curdate(),interval 1 day)" %(e[0],e[1])     
    cur.execute(update)
    conn.commit()
