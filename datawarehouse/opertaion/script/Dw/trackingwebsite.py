...
def upload_to_gs(db):
    ...
    end = datetime.date(2016,4,28)
    for i in xrange((end- beg).days+1):
	date = str(end - datetime.timedelta(days=i))
	date1 = str(end - datetime.timedelta(days=i-1)) 
	f = "trackingWebsite%s.csv" %date.replace("-","")
        cmd_csv = """mysql -heqx-vtweb-subordinate-db -ukettle -pk3UTLe tracker2 -e"select * from mddb.trackingWebsite where created_at >= '%s' and created_at < '%s';" >> %s""" %(date, date1, f)
#        print cmd_csv
	cmd_upload = "gsutil cp %s gs://vobile-data-analysis/OperationalMetrics/trackingWebsite" %f
	os.system(cmd_csv)
