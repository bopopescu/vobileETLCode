...
def upload_to_gs(db):
	...
	f = "matchedVideo_linkURL%s.csv" %date.replace("-","")
        cmd_csv = """mysql -heqx-vtweb-subordinate-db -ukettle -pk3UTLe tracker2 -e"select * from matchedVideo_linkURL where created_at >= '%s' and created_at < '%s';" >> %s""" %(date, date1, f)
        print cmd_csv
	cmd_upload = "gsutil cp %s gs://vobile-data-analysis/OperationalMetrics/matchedVideo_linkURL" %f
	os.system(cmd_csv)
