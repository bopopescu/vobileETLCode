...
dlcur=conn.cursor()
dlcur.execute('select website_type,date(report_at),sum(discovery_num) downloads from videoDailyStat a,webSite b where trackingWebsite_id=b.id and report_at >= "2016-03-01" and report_at < "2016-03-11" and b.website_type in ("hybrid") group by 1,2')
...
#removed
rmcur=conn.cursor()
rmcur.execute('select b.website_type, date(a.takeoff_time), count(*) removed from matchedVideo a, mddb.trackingWebsite b where a.company_id = 14 and a.trackingWebsite_id = b.id and b.website_type in ("hybrid") and a.takeoff_time >= "2015-07-01" and a.takeoff_time < "2016-03-01" group by 1,2')
