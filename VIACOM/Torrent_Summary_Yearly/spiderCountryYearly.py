...
class DBOperation:
    ...
    def InsertDate(self, site, YM, display_name, rank, country, topOneCountry):
        ...
        sql = "insert into Website_Alexa_Country_Yearly(site, YM, ETL_DTE, AlexaRank, AlexaCountry, PrimaryHostCountry, DisplayName) values('%s', '%s','%s','%s','%s','%s','%s');" %(site, YM, createdAt, rank, topOneCountry, country, display_name)
        print sql 
        cur.execute(sql)
...
def getInfo(db):
    ...
    sql = "select YM, websiteName from Torrent_Summary_Yearly_Tmp";
    cur.execute(sql)
    result = cur.fetchall()
...
def runAgain(db):
    cur, conn = db.connectMySQL()
    sql = "select site from Website_Alexa_Country_Yearly where AlexaRank = 0;" 
    cur.execute(sql)
    ...
    for i in xrange(0, len(d_names)):
        ...
        url = baseUrl + site
        rank, country, topOneCountry = parserUrl(url)
	sql = "update Website_Alexa_Country_Yearly set AlexaRank = '%s', AlexaCountry = '%s', PrimaryHostCountry = '%s' where site = '%s';" %(rank, topOneCountry, country, site)
        print sql
        cur.execute(sql)
        time.sleep(10)
    db.closeMySQL(cur, conn)
