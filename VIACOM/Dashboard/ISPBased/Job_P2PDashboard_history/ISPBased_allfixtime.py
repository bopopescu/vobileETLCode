...
def loadingDaily(ETL_DTE):
    ...
    while retryTime < 10: 
        try:
            ...
            print e
            retryTime += 1
    cursor.execute("truncate table allP2P_Tmp_150304;drop index meta_isp on allP2P_Tmp_150304")
    cursor.close()
    #DASHBOARD.commit()
...
def main():
    step = 0
    while True:
        dat = time.strftime('%Y-%m-%d', time.localtime(1426057867.500153 - step * 24 * 60 * 60))
        print dat
        step += 1
        print step
        if '2015-03-01' == dat:
