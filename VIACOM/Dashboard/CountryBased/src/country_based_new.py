...
class CountryBased(object):
	...
	try:
	    ...
	    sql = """select "%s", hostCountry, title, ifnull(b.nationalFlag, 'unknown'), sum(allMatch), sum(infringing), \
		     ...
		     then IPs else 0 end) as infringingIPs from ISPBased where dateID = '%s' and CASFlag <> 1 group by 1,2) a \
		     left join (select nationalFlag,countryName from TitleBasedCountry where countryName <> 'unknown') b \
	...
	for e in res:
        ...
        message["Subject"] = "Viacom Dashboard CountryBased Daily Job"
