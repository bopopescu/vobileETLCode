...
log_file = '/Job/FOX/Dashboard/TitleBased/log/SiteBased.log'
...
try:
	...
	aggregate_TitleBased1_SQL = """
		select
		        ...
		        a.title,
		        sum(a.matchedNum),
		        sum(a.matchedNumDurationNoZero),
		        sum(a.infringingNum),
		        sum(a.infringingNumDurationNoZero),
		        sum(a.infringingNumCMS),
		        sum(a.clipDurationSum),
		        sum(a.clipDurationInfringingSum),
        		sum(a.reportedViews),
        		sum(a.infringingViews),
        		sum(a.reportedViewsCMS),
        		a.ETLDate
		from
		(select
        		a.reportDate as reportDate,
		        a.trackingWebsite_id as trackingwebsite_id,
		        b.websiteName as websiteName,
		        b.websiteType as websiteType,
		        b.websiteDomain as websiteDomain,
...
try:
	...
	aggregate_TitleBasedRemoveNum_SQL = """
		select
			...
			CURRENT_TIMESTAMP as ETLDate
		from FOX_DASHBOARD.TitleBasedRemoveNum as a, FOX_DASHBOARD.TitleBasedTrackingWebsite as b
		where a.trackingWebsite_id = b.trackingWebsite_id
