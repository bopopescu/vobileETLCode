...
IDC="WASU"
STARTDATE = time.strftime('%Y-%m-%d',time.localtime(time.time()- 24*60*60))
...
"""
wasu_update_sql="""update OPM_MediaWise set task_pending_num=task_submitted_num-task_finished_num where report_at='%s' and data_center='WASU';"""
...
try:
	...
	str_wsql1="""insert ignore into OPM_MediaWise(data_center,company_name, task_submitted_num, task_finished_num, task_failed_num, task_matched_num, task_noresult_num, task_avg_duration, task_avg_response, report_at) values('%s', '%s', %s, %s, %s, %s, %s, %s, %s, '%s')""" %('WASU',row[0] ,row[1],row[2],row[3],row[4],row[5],row[6],row[7],STARTDATE)
    ...
    for row in str_wresult2:
        str_wsql2="""insert ignore into OPM_MediaWise(data_center, company_name, task_abnormal_num, report_at) values ('%s', '%s', %s, '%s') ON DUPLICATE KEY UPDATE task_abnormal_num=%s""" %('WASU',row[0] ,row[1], STARTDATE, row[1])
