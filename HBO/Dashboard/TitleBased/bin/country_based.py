...
conf.read("/Job/HBO/Dashboard/TitleBased/conf/viacom_dashboard.cfg")
...
class CountryBased(object):
    ...
    def connectTargetDB(self):
        try:
			...
			passwd = conf.get("target_server_staging", "passwd"), db = "HBO_DASHBOARD")
            cur = conn.cursor()
...
def main():
    logging.basicConfig(level=logging.DEBUG,
                ...
                filename='/Job/HBO/Dashboard/TitleBased/log/countryBased.log',
                filemode='a')
    c = CountryBased()
    c.ETL_history()
    c.send()
