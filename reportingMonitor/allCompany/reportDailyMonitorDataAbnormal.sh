...
manwin2_InfringingVideosFound_abnormalKPI=`$mysql_conf_114 DM_MANWIN2 -e "SELECT ROUND(SUM(totalNum)*100*${comparisonInterval}/(SELECT SUM(totalNum) FROM  InfringingVideosFound WHERE DateID < '$yesterday' AND DateID >= DATE_ADD('$yesterday', INTERVAL -${comparisonInterval} DAY))) AS '' FROM InfringingVideosFound WHERE DateID = '$yesterday';"`

##InfringingVideosRemoveOrNot
manwin2_InfringingVideosFound_abnormalKPI=`$mysql_conf_114 DM_MANWIN2 -e "SELECT ROUND(SUM(infringRemoveNum+infringNotRemoveNum)*100*${comparisonInterval}/(SELECT SUM(infringRemoveNum+infringNotRemoveNum) FROM  InfringingVideosRemoveOrNot WHERE DateID < '$yesterday' AND DateID >= DATE_ADD('$yesterday', INTERVAL -${comparisonInterval} DAY))) AS '' FROM InfringingVideosRemoveOrNot WHERE DateID = '$yesterday';"`

##NoticeSendDaily
manwin2_NoticeSendDaily_abnormalKPI=`$mysql_conf_114 DM_MANWIN2 -e "SELECT ROUND(SUM(totalSendNum)*100*${comparisonInterval}/(SELECT SUM(totalSendNum) FROM  NoticeSendDaily WHERE DateID < '$yesterday' AND DateID >= DATE_ADD('$yesterday', INTERVAL -${comparisonInterval} DAY))) AS '' FROM NoticeSendDaily WHERE DateID = '$yesterday';"`
